...
import { loginOut } from "@src/store/actions/userInfo";
...
const My: FC<IMyProps> = ({ loginname, theme, setTheme, loginOut }) => {
  ...
  // 数据
  const { data, loading } = useURLLoader("user", JSON.stringify(params), !params.loginname);
  // 判断是否是登录的自己
  useEffect(() => {
  ...
  return (
    <>
      {loading ? (
        ...
        <div className="sz-my" style={{paddingBottom: loginname === data.loginname ? 48 : 0}}>
          <Theme
            visible={visible}
                  ...
                  <List>
                    <ListItem
                      ...
                      after={
                        <div>
                          {theme.name}
                          <span className="ued-mobile">&#xe6b1;</span>
                        </div>
                      }
                    >
                      <div>换肤</div>
          ...
          {loginname === data.loginname ? <Footer value="my" /> : null}
        </div>
      )}
    </>
  );
