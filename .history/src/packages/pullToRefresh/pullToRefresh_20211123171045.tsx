...
const PullToRefresh: FC<IPullToRefreshProps> = ({
  ...
  const touchendEvent = () => {
    if (pullTorefresh.current !== null) {
      console.log(touch.transition, 'touch.transitiontouch.transition')
      if (touch.transition >= 55) {
  ...
  const touchstartEvent = (e: any) => {
    if (pullTorefresh.current !== null) {
      ...
      if (getScrollTop() <= 0) {
        ...
        pullTorefresh.current.removeEventListener("touchmove", touchmoveEvent, false);
        pullTorefresh.current.removeEventListener("touchend", touchendEvent, false);
      }
      setTouch(() => {
        // startY用来计算距离
