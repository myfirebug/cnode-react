...
const PullToRefresh: FC<IPullToRefreshProps> = ({
  ...
  const setTranslate = (y=0, durutiion=0) => {
      if (pullTorefresh.current !== null) {
        pullTorefresh.current.style.webkitTransitionDuration = `${durutiion}ms`;
        pullTorefresh.current.style.transitionDuration = `${durutiion}ms`;
        pullTorefresh.current.style.webkitTransform = `translate(0px, ${y}px) translateZ(0px)`;
