...
const ScrollLoad = memo((props: IScrollLoad) => {
        ...
        .then((res) => {
          if (res.success) {
            setData((state: IData) => {
              ...
              error.current = false;
              noMore.current = currPage.current === 100;
              return {
  ...
  return (
    <div className="sz-scroll-load">
      <p>{`当前${currPage.current}页`}</p>
      <p>{`当前loading状态：${String(loading.current)}`}</p>
      <p>{`当前error状态：${String(error.current)}`}</p>
      <p>{`当前noMore状态：${String(noMore.current)}`}</p>
      {noMore.current ? <div>--到底了--</div> : null}
      {render &&
        render(data)}
