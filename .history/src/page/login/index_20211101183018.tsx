...
import "./index.scss";
import { Input, Toast } from "@components/index";
import Ajax from "@service/index";
import SM4Util from "@util/sm4";
...
const Login: React.FC = (props) => {
    ...
    if (validate()) {
      if (!switchType) {
        Ajax.rsa().then((res) => {
          if (res.success) {
            const data = res.data;
            const s4 = new SM4Util(data);
            Ajax.passwordLogin({
              username: params.phone,
              password: s4.encryptData_CBC(params.password),
              publicKey: data,
            }).then((res) => {
              if (!res.success) {
              }
  ...
  }, [validate, switchType, params.phone, params.password, params.code]);
