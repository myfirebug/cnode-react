      ...
      {loading ? (
        ...
        <div className="sz-details">
          ...
          <div className="sz-reply">
            <div className="sz-reply__header">
              <h2 className="title">评论（{data.reply_count}）</h2>
            </div>
            <div className="sz-reply__list">
              {
                data.replies ?
                data.replies.map((item:IReplyProps, index: number) => (
                  <Reply {...item} key={item.id} index={index + 1} />
                )) : null
              }
              {
                data.replies && !data.replies.length ?
                <div className="sz-no-data">暂无数据</div> : ''
              }
            </div>
          </div>
        </div>
